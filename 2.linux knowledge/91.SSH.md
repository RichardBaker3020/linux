...
Both parties agree on a large prime number, which will serve as a seed value, they also agree on an encryption generator (typically AES).
Each party comes up with another prime number which is kept secret from the other party. This number is used as the private key for this interaction (different than the private SSH key used for authentication
