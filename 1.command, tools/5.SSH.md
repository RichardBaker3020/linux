...
### ssh-agent  ssh-add
ssh-agent 和 ssh-add 是 SSH 协议套件的标准组件，用于管理私钥。一般情况下我们使用不带密码的
id\_rsa 作为我们的默认私钥，此时是没必要启动 ssh-agent
的。当我们碰到以下两种情况则需要它：

1.  使用不同的秘钥连接到不同的主机时，需要手动指定对应的秘钥。（ssh-agent
    帮我们选择对应的秘钥进行认证）
2.  当私钥设置了密码，而我们又需要频繁的使用私钥进行认证。（ssh-agent
    帮我们免去重复输入密码）
