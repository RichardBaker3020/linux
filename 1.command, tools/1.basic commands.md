...
Or we could have a command that outputs shell commands. Running `ssh-agent` starts the agent in the background, and outputs a bunch of variable assignments:
```
...
which could be set in the current shell
and used by child processes (the `ssh` commands you would run).
...
And the current shell will get the variables for other commands to
inherit.
...
```
sudo mount -o remount,rw /dev/nvme0n1p3 /media/patrick/系统
```
