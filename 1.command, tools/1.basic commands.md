...
It will *not* create a child process, so the variable is set in the
current shell. (Of course `eval /bin/ls` will create a child process,
the same way a plain old `/bin/ls` would.)
