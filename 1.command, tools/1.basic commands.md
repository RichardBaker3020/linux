...
Of course, if the variable `cmd` happened to contain something
like `rm -rf $HOME`, then running `eval "$cmd"` would not be something
you'd want to do. Even things like command substitutions inside the
string would be processed, so *one should **really** be sure that the
input to `eval` is safe before using it.*
