...
Linux kernel distinguishes between two types of network interfaces: *physical* and *virtual*. *Physical network* interface represents an actual network hardware device such as network interface controller (NIC). *Virtual network* interface doesn’t represent any hardware device and is usually linked to one. 
